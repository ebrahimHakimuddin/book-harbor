...
@Composable
fun LibraryScreen(state: LibraryUiState, onServer: (String) -> Unit, onSignIn: (String, String, String) -> Unit, onRetry: () -> Unit, onSignOut: () -> Unit, onDownload: (Edition) -> Unit = {}, onRemove: (Edition) -> Unit = {}) {
...
@Composable private fun SignInForm(onSignIn: (String, String, String) -> Unit) { var email = androidx.compose.runtime.remember { androidx.compose.runtime.mutableStateOf("") }; var password = androidx.compose.runtime.remember { androidx.compose.runtime.mutableStateOf("") }; OutlinedTextField(email.value, { email.value = it }, label = { Text("Email") }, modifier = Modifier.fillMaxWidth()); OutlinedTextField(password.value, { password.value = it }, visualTransformation = PasswordVisualTransformation(), label = { Text("Password") }, modifier = Modifier.fillMaxWidth()); Button(onClick = { onSignIn(email.value, password.value) }, enabled = email.value.isNotBlank() && password.value.isNotBlank()) { Text("Sign in") } }
